...
def plus_unlimited(*args, **kwargs):
  '''
      *args과 **kwargs라 딕셔너리의 형태로 값을 받는 함수
      전체 합의 결과를 출력함
  '''
...
'''
    키가 180cm 이상인 사람들의 경우 "키가 정말 큽니다."라는 메세지가 출력하고,
...
'''
def height_high(num):
  return "키가 정말 큽니다." if num >= 180 else "계속 키가 클 겁니다."
# if num >=180: 
#  return "키가 정말 큽니다."
# else:
#  return "계속 키가 클 겁니다."
...
'''
    다섯 사람의 키입니다.
    height_hugh() 함수를 이용하여 실행해보기

    map(함수, 입력들)
    map(functionm *iterables) 현태로 사용가능함
      - *iterables : 원소를 하나씩 차례로 반환이 가능한 object를 의미함 (list, str, tuple)
'''
def calc(x):
  return x * x
